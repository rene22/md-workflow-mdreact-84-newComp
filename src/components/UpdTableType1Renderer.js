...
function UpdTableType1Renderer(props) {
    const { classes } = props;
    var numOfCols = 0;

    props.gridList.filter((table) => props.frame.tableID === table.id).forEach((table) => (
                ...
                <CardContent className={classes.updatableTableCardContent}>
                    {props.gridList.filter((table) => props.frame.tableID === table.id).map((table) => (
                        table.rows.row.map((row, idx1) => (
                            <Grid container key={table.id + idx1} className={classes.updatableTableGridRow}>
                                {row.item.map((col, idx2) => (
                                    col.visible && col.visible === 'false' ?
                                        <FieldRenderer
                                            key={col.id+idx1+idx2}
                                            idx={idx1+1}
                                            item={Object.assign(col, { label: table.header.item[idx2].value })}
                                        ...
                                        <Grid item className={classes.updTabGridItem} key={table.id + idx1 + idx2}>
                                            <FieldRenderer
                                                key={col.id+idx1+idx2}
                                                idx={idx1+1}
                                                item={Object.assign(col, { label: table.header.item[idx2].value })}
...
export default UpdTableType1Renderer;