...
function evaluateJson(jsonObj) {
  Object.keys(jsonObj).forEach((attr) => {
    if (typeof jsonObj[attr] === 'object') {
      ...
      jsonObj[attr] =
        isNaN(jsonObj[attr]) && jsonObj[attr].search('`') >= 0
          ? eval(jsonObj[attr])
          : jsonObj[attr];
    }
  });
...
function loadFromLocalStorage() {
  return new Promise((resolve, reject) => {
    try {
      ...
      if (
        localStorage.getItem('sessionRetryCount') &&
        localStorage.getItem('sessionRetryCount') >= 3
      )
        ...
        .then(async (themeData) => {
          ...
          fetch('./components.json')
            ...
            .then(async (componentsData) => {
              ...
              styles.content.transition = `${theme.transitions.create(
                'margin',
                styles.content.transition
              )}ms`;

              styles.contentShift.transition = `${theme.transitions.create(
                'margin',
  ...
  return (
    <CookiesProvider>
      <Provider store={store}>
        {theme && styles ? <App theme={theme} styles={styles} /> : ''}
      </Provider>
    </CookiesProvider>
  );
};
