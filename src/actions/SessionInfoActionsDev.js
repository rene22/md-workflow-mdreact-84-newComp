...
export const establishSession = () => async (dispatch, getState) => {
  return new Promise(async (resolve, reject) => {
    ...
    var startupConfigs = state.serverReducer.startupConfigs
      ? state.serverReducer.startupConfigs
      : undefined;
    ...
    if (count < 3) {
      fetch('./startup.json')
        ...
        .then(async (startupConfigs) => {
          ...
          var url =
            startupConfigs.deploymentMode === 'server'
              ? '../../MDREP' +
                startupConfigs.sessionPayload.environment +
                '/' +
                startupConfigs.apiPaths.MRDCONREP
              : startupConfigs.apiPath +
                'MDREP' +
                startupConfigs.sessionPayload.environment +
                '/' +
                startupConfigs.apiPaths.MRDCONREP;
          ...
          if (startupConfigs.debugMode.debugOn)
            console.log('Sending session establishment request: \n', bodyParam);

          await apis
            .post(url, bodyParam, axiosConfig, { timeout: 15000 })
            .then((resp) => {
              response = resp;
              ...
              if (response && response.status === 200) {
                ...
                if (startupConfigs.debugMode.debugOn)
                  console.log(
                    'Session establishment response from server: \n',
                    response
                  );
                ...
                dispatch(
                  setDialogPopUp({
                    ...
                    content: {
                      ...
                      details:
                        'Recovery: Please check the HTTP server instance is started and correctly configured',
                    },
                    action: 'Reload',
                  })
                );
                ...
                dispatch(
                  setDialogPopUp({
                    ...
                    content: {
                      title: `Message: ${resp.status} ${resp.statusText} - ${resp.request.responseURL}`,
                      details:
                        'Recovery: Please fix the error in startup.json config file and reload the page',
                    },
                    action: 'Dismiss',
                  })
                );
                ...
                dispatch(
                  setDialogPopUp({
                    ...
                    content: {
                      ...
                      details:
                        'Recovery: Select Reload to logoff and login again.',
                    },
                    action: 'Reload',
                  })
                );
            ...
            .catch((err) => {
              if (startupConfigs.debugMode.debugOn)
                console.log('Exception error: ', err.response);
              ...
              dispatch(
                setDialogPopUp({
                  ...
                  content: {
                    ...
                    details:
                      'Recovery: Please check the Server status or correct the error in startup.json config file and reload the page',
                  },
                  action: 'Dismiss',
                })
              );
      ...
      console.log(
        'Max. session establishment retries reached, please try after sometime.'
      );
